...
test.describe("OpenYak complete GUI workflows", () => {
  ...
  test("chat task journey: workspace, attachment, mention, send, persist, search, reopen", async ({ page }) => {
    ...
    expect(JSON.stringify(state.promptBodies[0])).toContain("/Users/alex/openyak-demo");
    ...
    await expect(page.getByLabel("Results").getByText("Create a UI preflight checklist")).toBeVisible();
    await page.getByLabel("Results").getByText("Create a UI preflight checklist").click();
    await expect(page).toHaveURL(/\/c\/session-new$/);
    await expectNoAppCrash(page);
  });

  test("provider setup journey: configure providers, switch model surface, send with selected provider", async ({ page }) => {
    const state = await setupMockedApp(page);
    ...
    const ownKeySave = page.waitForResponse((res) =>
    ...
    await expect
      .poll(() => page.evaluate(() => JSON.parse(window.localStorage.getItem("openyak-settings") ?? "{}")?.state?.activeProvider))
      .toBe("byok");
    ...
    await sendPrompt(page, "Create a UI preflight checklist");
    await expect(page).toHaveURL(/\/c\/session-new$/);
    expect(JSON.stringify(state.promptBodies[0])).toContain("openrouter/anthropic/claude-sonnet-4.5");
    ...
    const localSave = page.waitForResponse((res) =>
      res.url().includes("/api/config/local") &&
      res.request().method() === "POST" &&
      res.status() === 200,
    );
    await page.getByRole("button", { name: "Save" }).click();
    await localSave;
    expect(JSON.stringify(state.providerSaves)).toContain("http://localhost:1234/v1");

    await page.goto("/settings?tab=providers");
    await page.getByRole("button", { name: /Custom Endpoint/i }).click();
    await page.getByPlaceholder("Endpoint Name (e.g. My Local Model)").fill("Workflow Endpoint");
    await page.getByPlaceholder("https://api.example.com/v1").fill("http://localhost:9888/v1");
    await page.getByPlaceholder("API Key (Leave blank if not required)").fill("sk-custom-workflow");
    const customSave = page.waitForResponse((res) =>
  ...
  test("automation lifecycle journey: create, run, inspect history, edit, delete", async ({ page }) => {
    const state = await setupMockedApp(page);
    ...
    await page.getByPlaceholder(/Brief description/i).fill("Watch docs before standup");
    ...
    expect(JSON.stringify(state.automationCreates[0])).toContain("Release note watcher");

    const createdCard = page.locator("div.rounded-lg").filter({ hasText: "Release note watcher" });
    ...
    await page.locator('input[type="text"]').first().fill("Release note watcher updated");
    await page.getByRole("button", { name: "Save" }).click();
    ...
    expect(JSON.stringify(state.automationUpdates)).toContain("Release note watcher updated");

    const updatedCard = page.locator("div.rounded-lg").filter({ hasText: "Release note watcher updated" });
  ...
  test("remote mobile handoff journey: enable desktop remote, change provider on mobile, submit task", async ({ page }) => {
    const state = await setupMockedApp(page);
    ...
    await page.getByRole("button", { name: /ChatGPT Subscription 1 model available/i }).click();
    await expect
      .poll(() => page.evaluate(() => window.localStorage.getItem("openyak_remote_provider")))
      .toBe("chatgpt");
    ...
    await expect(page).toHaveURL(/\/m\/task\/_\?sessionId=session-new&stream_id=stream-ui-1$/, { timeout: 20_000 });
    expect(JSON.stringify(state.promptBodies[0])).toContain("Check the release notes from my phone");
    expect(JSON.stringify(state.promptBodies[0])).toContain("openai-subscription/gpt-5.5");
    await expectNoAppCrash(page);
  });
});
